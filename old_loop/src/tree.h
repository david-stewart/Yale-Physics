...
/*     /1* cout << "d0" << endl; *1/ */
/*     /1* for (auto i : options) cout << " > " << i << "<|" << endl; *1/ */

/*     /1* flog = fopen(log_name.Data(),"w"); *1/ */
/*    if (tree == 0) { */
...
/*     /1* fprintf(flog, " # %8.4f sec/(1M events) \n", 1E6* f_seconds / (float) n_calc_events); *1/ */
/*     /1* std::cout << "n_calc_events " << n_calc_events << " " << difftime(end_time, start_time) << std::endl; *1/ */
