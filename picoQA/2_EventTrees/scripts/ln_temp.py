...
for fname in glob("../../out/P17/152EB*"):
    ...
    if b_no_WeEND or has_bad_alloc:
        ...
        call(['ln','%s.root'%e_name,'%s/%s.root'%(ln_dir,e_name)])
        ngood += 1


fout = open('%s/readme.md'%ln_dir,'w')
...
for L in open('%s/readme.md'%ln_dir):
    print L.strip()
